...
router.post('/sale_item', (req, res, next) => {
    mysql.getConnection((error, conn) => {
        if (error) { return res.status(500).send({ error: error }) }
        conn.query(
            'INSERT INTO sale_item (product_id_product_pdt, sale_id , quantity_item, price_item ) VALUES (?,?,?,?)',
           [req.body.product_id_product_pdt, req.body.sale_id, req.body.quantity_item,req.body.price_item],
